...
- A new Checkout item is used for the variant-based workflow. Its item and variant IDs must be captured after creation and added to `src/lib/receipts/open-play-items.ts` without removing the historical IDs.
- Checkout SKUs: `FLEXI-CHECKOUT-HOURS-01` through `08`.
- Entrance SKUs: `FLEXI-ENTRANCE-KIDS-01` through `05`.
- The new Entrance item and its variant IDs must also be captured after creation.
...
2. In Loyverse Back Office, create a new `Flexi Checkout` item in `Entry`, with option `Hours punched this visit`, values `1 hour` through `8 hours`, fixed price `0`, and the stable Checkout SKUs.
3. In Loyverse Back Office, create `Flexi Entrance` in `Entry`, with option `Number of kids entering`, values `1 kid` through `5 kids`, fixed price `0`, and the stable Entrance SKUs.
...
5. Replace both `ID` and `LoyverseID` on the two Notion rows with the new item IDs. Write the returned variant IDs into `Variants JSON`. The Checkout row must no longer point to the historical item.
