...
export class LegacyTransferSlipParser implements ExpenseParser {
  id = 'kbank-transfer-legacy';
  name = 'K-Bank Transfer Slip (Legacy)';
  ...
  validate(rawText: string): boolean {
    ...
    const hasThaiKeyword = 
      ...
      text.includes('โ อ น เ ง น ส ำ เ ร จ');
