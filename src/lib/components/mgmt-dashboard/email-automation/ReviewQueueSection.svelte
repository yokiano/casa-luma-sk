<script lang="ts">
  import { ChevronDown, ClipboardList, Copy } from 'lucide-svelte';
...
<details class="group overflow-hidden rounded-3xl border border-amber-200 bg-white shadow-sm">
  <summary class="flex cursor-pointer list-none items-center justify-between gap-3 bg-amber-50 px-5 py-4">
    <div>
    ...
    <div class="flex shrink-0 items-center gap-2">
      ...
      <ChevronDown size={18} class="text-amber-800 transition-transform group-open:rotate-180" />
    </div>
  </summary>
  <div class="flex justify-end border-t border-amber-100 bg-white px-5 py-3">
  ...
  <div class="border-t border-amber-100 bg-white px-5 py-3">
  ...
  {#if reviews.length === 0}
    <p class="border-t border-amber-100 px-5 py-8 text-sm text-[#7a6550]">No open email reviews. Historical review records are retained as done when completed.</p>
  {:else}
    <div class="space-y-2 border-t border-amber-100 bg-[#fbf8f4] p-2">
