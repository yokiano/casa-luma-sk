...
<footer class="bg-muted/30 border-t border-border mt-20">
	<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
		<div class="grid grid-cols-1 md:grid-cols-3 gap-8">
			<!-- Brand -->
			<div>
				<h3 class="text-lg font-semibold mb-3">{BUSINESS_INFO.name}</h3>
				<p class="text-sm text-muted-foreground mb-2">{BUSINESS_INFO.tagline}</p>
				<p class="text-sm text-muted-foreground">{BUSINESS_INFO.location}</p>
			</div>

			<!-- Contact -->
			<div>
				<h3 class="text-lg font-semibold mb-3">Contact</h3>
				<p class="text-sm text-muted-foreground mb-1">{BUSINESS_INFO.email}</p>
				<p class="text-sm text-muted-foreground">{BUSINESS_INFO.phone}</p>
			</div>
