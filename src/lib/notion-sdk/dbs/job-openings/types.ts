...
export interface JobOpeningsResponse extends WithOptional<Omit<DatabaseObjectResponse, 'properties'>, 'title'| 'description'| 'is_inline'| 'url'| 'public_url'> {
  properties: {
    ...
    "Required Skills": Omit<MultiSelectPropertyItemObjectResponse, 'multi_select'> & { multi_select: [{ id: StringRequest, name: 'Project Management', color: 'purple' } | { id: StringRequest, name: 'Leadership', color: 'red' } | { id: StringRequest, name: 'Problem Solving', color: 'green' } | { id: StringRequest, name: 'English', color: 'yellow' } | { id: StringRequest, name: 'Handyman', color: 'default' }]},
