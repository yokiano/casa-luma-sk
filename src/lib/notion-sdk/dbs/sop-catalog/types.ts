...
export interface SopCatalogResponse extends WithOptional<Omit<DatabaseObjectResponse, 'properties'>, 'title'| 'description'| 'is_inline'| 'url'| 'public_url'> {
  properties: {
    ...
    "SOP Type": Omit<SelectPropertyItemObjectResponse, 'select'> & { select: { id: StringRequest, name: 'Checklist', color: 'gray' } | { id: StringRequest, name: 'How-to', color: 'blue' } | { id: StringRequest, name: 'Policy / management procedure', color: 'purple' } | { id: StringRequest, name: 'Training / onboarding', color: 'green' }},
...
export type SopCatalogPropertyFilter = { role: SopCatalogRolePropertyFilter } | { when: SopCatalogWhenPropertyFilter } | { status: SopCatalogStatusPropertyFilter } | { sopType: SopCatalogSopTypePropertyFilter } | { department: SopCatalogDepartmentPropertyFilter } | { name: SopCatalogNamePropertyFilter }
