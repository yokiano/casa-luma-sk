...
export interface EmployeesResponse extends WithOptional<Omit<DatabaseObjectResponse, 'properties'>, 'title'| 'description'| 'is_inline'| 'url'| 'public_url'> {
  properties: {
    ...
    "Department": Omit<SelectPropertyItemObjectResponse, 'select'> & { select: { id: StringRequest, name: 'Café', color: 'purple' } | { id: StringRequest, name: 'Open Play', color: 'brown' } | { id: StringRequest, name: 'Marketing', color: 'green' } | { id: StringRequest, name: 'Maintenance', color: 'pink' } | { id: StringRequest, name: 'Management', color: 'gray' }},
