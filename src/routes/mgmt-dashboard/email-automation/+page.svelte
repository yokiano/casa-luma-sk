<script lang="ts">
  ...
  const roadmap: RoadmapItem[] = [
    ...
    { phase: 'Security 2A', title: 'Sender-authenticity observation', status: 'partial', delivered: 'Unknown authenticity is visible. Production writes require the dashboard switch, visible-sender allowlist, complete MIME, extracted reference and amount, and a dashboard max amount limit.', missing: 'SPF, DKIM, DMARC, ARC, Return-Path and visible-From alignment are not yet captured from routed mail.', next: 'Capture bounded authentication evidence from representative real emails in report-only mode.', gate: 'Requires Cloudflare Worker deployment preparation and real routed-email samples.' },
    { phase: 'Security 2B', title: 'Sender-authenticity enforcement', status: 'blocked', delivered: 'Ledger execution uses explicit production safeguards; the emergency stop is the dashboard Ledger switch.', missing: 'No approved authenticated-domain policy or forwarding exception policy exists yet.', next: 'Review report-only evidence, approve policy, then quarantine failed or indeterminate financial senders.', gate: 'Do not widen the sender allowlist until real header evidence proves legitimate mail passes.' },
    ...
    { phase: 'Recovery', title: 'Retries, temporary work reservations and reconciliation', status: 'partial', delivered: 'Manager action retry, Telegram-only retry, temporary work reservations that expire after an interrupted processor, stale-completion protection and attempt history exist. Ledger recovery uses the same production safety policy as intake and execution.', missing: 'There is no automatic retry timer. Today, due work is visible here and available recovery controls are shown on the event page.', next: 'If automatic retries are later approved, add a bounded Vercel Cron processor every 5 minutes and record each run, including idle runs.', gate: 'Recommended choice: Vercel Cron. Configure a private CRON_SECRET bearer token and deploy the endpoint. This processes retries only; it cannot bypass Ledger safety controls.' },
    ...
    { phase: 'Rollout', title: 'Production verification', status: 'partial', delivered: 'Migrations through 0015 are applied. This dashboard reads the production database and shows Ledger safety controls, the separate attention queue, live queue health and recent email events.', missing: 'Automatic retry scheduling and some provider/runtime failure exercises remain future operational work.', next: 'Watch representative emails, open each event from this page, and follow its four facts: what happened, action taken, current state and next step.' }
  ];
...
<section class="space-y-5">
  ...
  <!-- Financial Ledger automation status -->
  <details class="group rounded-3xl border border-[#dfd2c5] bg-white shadow-sm">
    <summary class="flex cursor-pointer list-none items-center justify-between px-5 py-4">
      <div>
        <span class="flex items-center gap-2 font-semibold"><Scale size={18} />Financial Ledger automation status</span>
        <span class={`ml-3 rounded-full border px-2 py-0.5 text-xs font-bold ${dashboard.ledgerPolicy.mode === 'active' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>{dashboard.ledgerPolicy.mode === 'active' ? 'active for allowed senders' : 'blocked closed'}</span>
      </div>
    ...
    <div class="grid gap-4 border-t border-[#eee5dc] p-5 xl:grid-cols-3">
      <article class="rounded-2xl border border-[#eee5dc] bg-[#fbf8f4] p-4 text-sm">
        ...
        <p class="mt-2 leading-6 text-[#7a6550]">Matching transaction emails can create or reconcile a Financial Ledger row only after all production safety controls pass.</p>
        <p class="mt-2 text-xs text-[#7a6550]">Maximum automatic amount: <b>{dashboard.ledgerPolicy.maxAmountThb.toLocaleString('en-US')} THB</b>.</p>
      </article>
      <article class="rounded-2xl border border-[#eee5dc] bg-[#fbf8f4] p-4 text-sm">
        <h3 class="font-semibold text-[#2c2925]">Current gates</h3>
        <ul class="mt-2 list-disc space-y-1 pl-5 text-[#7a6550]">
          <li>Dashboard Ledger automation: <b>{dashboard.ledgerPolicy.dashboardLedgerEnabled ? 'on' : 'off'}</b></li>
      ...
      <article class="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
        ...
        <p class="mt-2 leading-6">Sender authenticity is still not SPF/DKIM/DMARC enforced. Keep the sender allowlist narrow and review new Ledger rows.</p>
  ...
  <details class="group rounded-3xl border border-[#dfd2c5] bg-white shadow-sm">
    ...
    <div class="border-t border-[#eee5dc] p-5">
      <div class="mb-5 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-900">
        ...
        <p class="mt-1 leading-6">Financial Ledger writes require the dashboard switch, sender allowlist, MIME completeness, extracted reference/amount, amount limit, and idempotency checks. Sender authenticity is still not enforced, so keep the allowlist narrow until real authentication evidence is approved.</p>
      </div>
      ...
      <div class="mt-5 rounded-2xl border border-[#dfd2c5] bg-white p-4 text-sm">
        ...
        <ol class="mt-2 list-decimal space-y-1 pl-5 text-[#7a6550]"><li>Receive an email, then open its event from the review queue or recent events.</li><li>Read the four outcome facts before taking any recovery action.</li><li>Keep review completion separate from Ledger and Telegram actions.</li><li>Verify the Ledger safety controls before enabling automatic writes.</li><li>Consider automatic retry scheduling and broader sender access only as separate future approvals.</li></ol>
      </div>
    </div>
  </details>
