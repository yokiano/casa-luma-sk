import { getPayForPlaySyncStatus, syncPayForPlayItems } from '$lib/pay-for-play-sync.remote';
import type { MenuItemExtended } from './pos-sync.svelte';
import type { SyncReport } from '$lib/menu-sync.remote';

export class PayForPlaySyncState {
  ...
  async fetchStatus() {
    ...
    this.error = null;
    try {
      const results = await getPayForPlaySyncStatus();
