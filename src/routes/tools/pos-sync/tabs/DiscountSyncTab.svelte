<script lang="ts">
  import { DiscountSyncState } from '../pos-sync-discounts.svelte';
  
  const state = new DiscountSyncState();
</script>
...
<div>
  <div class="flex justify-between items-center mb-6">
    <div>
      ...
      <p class="text-gray-500 text-sm">Sync Notion Discounts to Loyverse</p>
    </div>
      ...
      <table class="w-full text-left text-sm">
        <thead class="bg-gray-50 border-b">
          <tr>
            ...
            <th class="p-4 font-medium text-gray-500">Value</th>
          ...
          {#each state.items as item}
            <tr class="hover:bg-gray-50 transition-colors">
              ...
              <td class="p-4 text-gray-600">
                {item.type === 'FIXED_PERCENT' ? 'Percentage' : 'Amount'}
              </td>
              <td class="p-4 text-gray-600">
                 {#if item.type === 'FIXED_PERCENT'}
                   {item.value}%
                 {:else}
                   {item.value}
                 {/if}
