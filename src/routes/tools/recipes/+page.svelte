		...
		try {
			...
			selectedRecipe = {
				...
				hasInstructions: Boolean(
					(language === 'english' ? text : selectedRecipe.instructionsText)?.trim() ||
					(language === 'thai' ? text : selectedRecipe.thaiInstructionsText)?.trim()
				),
				isComplete:
					selectedRecipe.hasIngredientLines &&
					Boolean(
						(language === 'english' ? text : selectedRecipe.instructionsText)?.trim() ||
						(language === 'thai' ? text : selectedRecipe.thaiInstructionsText)?.trim()
					)
			...
			{:else if selectedRecipe}
				<article class="overflow-hidden rounded-3xl border border-[#dfd2c6] bg-white shadow-sm">
					...
					<section class="border-t border-[#eee4da] p-4 md:p-5">
						...
						{#if !selectedRecipe.instructionsText && !selectedRecipe.thaiInstructionsText}
